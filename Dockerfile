...
RUN chmod +x /docker-entrypoint.d/05-data-init.sh /docker-entrypoint.d/10-admin-auth.sh /docker-entrypoint.d/20-alert-token.sh /docker-entrypoint.d/25-snapshot-config.sh /docker-entrypoint.d/30-clock-config.sh /docker-entrypoint.d/40-weather-fetch.sh && printf '{}\n' > /usr/share/nginx/html/snapshot.json
