...
fetch_loop() {
  while true; do
    ...
    if [ -n "$LAT" ] && [ -n "$LON" ]; then
      URL="https://api.open-meteo.com/v1/forecast?latitude=$LAT&longitude=$LON&current=temperature_2m,apparent_temperature,relative_humidity_2m,weather_code,precipitation&daily=temperature_2m_max,temperature_2m_min,sunrise,sunset&hourly=precipitation_probability&temperature_unit=celsius&timezone=auto&forecast_days=1"
